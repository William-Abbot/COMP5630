...
def hashboard(state):
    ...
    boardhash = "".join(code[state.at(loc).color] for loc in
                        product(range(state.size), repeat=2))
...
class Board(object):
    def __init__(self):
        ...
        self.chains = []
        self.board = [[Empty((i, j)) for j in range(BOARD_SIZE)] for i in range(BOARD_SIZE)]
        self.stable_areas = {"black":[[0 for j in range(BOARD_SIZE)] for i in range(BOARD_SIZE)],
            "white":[[0 for j in range(BOARD_SIZE)] for i in range(BOARD_SIZE)]}
        self.history = set()
        self.last_move = None
    ...
    def find_stable_areas(self):
        ...
        self.stable_areas = {"black":[[0 for j in range(BOARD_SIZE)] for i in range(BOARD_SIZE)],
            "white":[[0 for j in range(BOARD_SIZE)] for i in range(BOARD_SIZE)]}
        eye_map = {"black":[[0 for j in range(BOARD_SIZE)] for i in range(BOARD_SIZE)],
            "white":[[0 for j in range(BOARD_SIZE)] for i in range(BOARD_SIZE)]}

        stable_index = {"black": set(), "white":set()}
        for loc in product(range(self.size), repeat=2):
...
def actions(state, color):
    choices = []
    for loc in product(range(state.size), repeat=2):
